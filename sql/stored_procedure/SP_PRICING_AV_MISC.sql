	...
	BEGIN
		...
		AS (
			...
			FROM TOOMISC TOOMISC
				,TABLE (func_miscvalid(IN_TOCODE, TOOMISC.MISCKEY, pricedatefrom, pricedateto, IN_NRADULTS, cdob1, cdob2, cdob3, cdob4)) AS x
				...
				,(
					CASE 
						WHEN (
							...
							THEN func_get_allotment2(IN_TOCODE, x.MISCKEY, 'M', pricedatefrom, x.ALLOTMENTENDDATE, currentdate)
						ELSE 'XX'
						END
					)
			FROM tmptable1 x
		...
		AS (
			...
			FROM tmptable2 x
				,TABLE (func_pricing2_tbl(IN_TOCODE, x.MISCKEY, 'M', pricedatefrom, x.PRICEENDDATE, currentdate, x.NRADULTS, x.CHDDOB1, x.CHDDOB2, x.CHDDOB3, x.CHDDOB4)) AS pricing
			GROUP BY x.MISCKEY
			)
