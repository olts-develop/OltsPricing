CREATE OR REPLACE PROCEDURE SP_PRICING_AV_HOTEL
(
...
select
  ...
  , (case upper(IN_LANGCODE) when 'EN' then h.HOTELNAMEEN when 'FR' then h.HOTELNAMEFR when 'IT' then h.HOTELNAMEIT else h.HOTELNAMEDE end ) as HOTELNAME
  ...
  , (case upper(IN_LANGCODE) when 'EN' then r.ROOMTYPEEN when 'FR' then r.ROOMTYPEFR when 'IT' then r.ROOMTYPEIT else r.ROOMTYPEDE end ) as ROOMTYPE
  , (case upper(IN_LANGCODE) when 'EN' then r.DESCRIPTIONEN when 'FR' then r.DESCRIPTIONFR when 'IT' then r.DESCRIPTIONIT else r.DESCRIPTIONDE end ) as DESCRIPTION
  , (case upper(IN_LANGCODE) when 'EN' then r.MEALDESCRIPTIONEN when 'FR' then r.MEALDESCRIPTIONFR when 'IT' then r.MEALDESCRIPTIONIT else r.MEALDESCRIPTIONDE end ) as MEALDESCRIPTION
...
from
  TOOHOTEL h
  inner join TOOROOMS r on h.HOTELKEY = r.HOTELKEY
  left outer join tmptble on h.HOTELKEY = XHOTELKEY and r.ROOMKEY = XROOMKEY
where
  h.DESTINATIONCODE = coalesce(nullif(IN_DESTINATIONCODE,''),h.DESTINATIONCODE)
  and h.HOTELCODE = coalesce(nullif(IN_HOTELCODE,''),h.HOTELCODE)
  and r.TOURBOCODE = coalesce(nullif(IN_ROOMCODE,''),r.TOURBOCODE)
  ...
  and h.HOTELKEY = coalesce(nullif(IN_HOTELKEY,''),h.HOTELKEY)
  and r.NORMALOCCUPANCY = IN_NORMALOCCUPANCY
  and ( (IN_IGNORE_PRICE0=1 and XPRICE > 0) or IN_IGNORE_PRICE0=0 )
  and coalesce(XALLTOMENTCODE,'XX') <> ( case when IN_IGNORE_XX=1 then 'XX' else '..' end)
  and coalesce(XALLTOMENTCODE,'XX') <> ( case when IN_IGNORE_RQ=1 then 'RQ' else '..' end)
order by
  (case IN_LANGCODE when 'EN' then h.HOTELNAMEEN when 'FR' then h.HOTELNAMEFR when 'IT' then h.HOTELNAMEIT else h.HOTELNAMEDE end )
  , h.HOTELKEY
  , coalesce(XPRICE,0) ;
