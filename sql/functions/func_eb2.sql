...
SELECT
  id
  ,datefrom
  ,( case when datefrom < p_startdate then p_startdate else datefrom end ) as effectivedatefrom
  ,dateto
  ,( case when dateto > returndateminus1 then returndateminus1 else dateto end ) as effectivedateto
  ,coalesce(percent,0) as percent
  ,coalesce(foralldays,0) as foralldays
  ,fromday
  ,today
  ,tmpday
  ,ROW_NUMBER() OVER (PARTITION BY id ORDER BY id ASC, tmpday ASC) as daterownr
...
WHERE
  ...
  AND tmpday BETWEEN p_startdate AND returndateminus1
  AND tmpday BETWEEN datefrom AND dateto
  AND percent <> 0
  ...
  (
    (
      ...
      AND p_currentdate BETWEEN datebeforedeparturefrom AND datebeforedepartureto
    )
    or
    (
      -- ( daysbeforedeparturefrom <> 0 OR daysbeforedepartureto <> 0 )
      daysbeforedeparturefrom + daysbeforedepartureto > 0
      AND p_currentdate BETWEEN p_startdate - daysbeforedeparturefrom days and p_startdate - daysbeforedepartureto days
    )
  )
  AND
  (
    (
      (
        ...
        OR (datefrom BETWEEN p_startdate AND returndateminus1 AND dateto BETWEEN p_startdate AND p_returndate - 1 DAY)
  ...
  (
    ...
    (
      foralldays = 0
      AND DAYS( case when dateto > returndateminus1 then returndateminus1 else dateto end ) - DAYS( case when datefrom < p_startdate then p_startdate else datefrom end ) BETWEEN fromday AND today
    )
  )
)
...
--  ,TABLE( func_pricing3_tbl (p_tocode, p_itemkey, p_startdate, p_returndate, p_currentdate, p_nradults, childbirthdate1, childbirthdate2, childbirthdate3, childbirthdate4 ) ) AS X
  ,TABLE( func_pricing3_tbl (p_tocode, p_itemkey, p_itemtype, p_startdate, p_returndate, p_currentdate, p_nradults, p_childbirthdate1, p_childbirthdate2, p_childbirthdate3, p_childbirthdate4 ) ) AS X   
WHERE
  ebrownr = 1
