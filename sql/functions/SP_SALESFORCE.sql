...
CREATE OR REPLACE PROCEDURE SP_SALESFORCE_CUSTOMER_ACTION
    ( IN ACTION        VARCHAR(26) DEFAULT 'GET' ,
      ...
      IN TYPE          VARCHAR(20) DEFAULT 'PRIVATE' ) DYNAMIC RESULT SETS 1 LANGUAGE SQL MODIFIES SQL DATA P1:
BEGIN
    ATOMIC
DECLARE
    NextCustomerNr INTEGER DEFAULT 0;
DECLARE
    Customer_Type_Seq VARCHAR(20) DEFAULT '';
DECLARE
    Payment_Condition_Seq VARCHAR(20) DEFAULT '';
    /* CENTRAL will fetch and increment the GLOBAL table, which BRANCH will fetch and UPDATE the REPGLOBAL for the branch */
DECLARE
    BIRTHDATE_AS_DATE DATE;
DECLARE
    Customer_Nr_Type VARCHAR(20) DEFAULT 'CENTRAL';
DECLARE
    ErrorCode VARCHAR(254) DEFAULT '';
DECLARE
    ErrorDesc VARCHAR(254) DEFAULT '';
    /* The cursor "cursor1" will be the result returned by the SQL in the case of success, and is identical to the result of a call to SP_SALESFORCE_CUSTOMER */
DECLARE
    cursor1
    CURSOR WITH
        RETURN FOR
...
DECLARE
    ...
    CURSOR WITH
        ...
        SELECT
            ACTION
AS
    "ACTION" , CUSTOMER_NR AS "CUSTOMER_NR" , 'error' as "STATUS" , ErrorCode AS "ERROR" , ErrorDesc AS "ERRORDESC" FROM SYSIBM.SYSDUMMY1;
