...
BEGIN
  ...
  SELECT p_tocode
    ,toorooms.roomkey
    ,func_pricing(p_tocode, toorooms.roomkey, p_startdate, p_returndate, p_currentdate, p_nradults, p_childbirthdate1, p_childbirthdate2, p_childbirthdate3, p_childbirthdate4, p_currency) AS pricetotal
    ,destinationcode
    ,hotelcode
    ,tourbocode
    ,mealcode
...
BEGIN
  ...
  SELECT TOCODE
    ...
    ,MEALCODE
    ,STATUS
  FROM TABLE (func_roompricebydest(p_tocode, p_destcode, startdate, returndate, currentdate, p_nradults, childbirthdate1, childbirthdate2, childbirthdate3, childbirthdate4, p_currency));
END
@
