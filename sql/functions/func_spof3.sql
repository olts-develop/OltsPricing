...
FROM
  (
    SELECT
      ROW_NUMBER() OVER(PARTITION BY so.childchildnr ORDER BY so.days desc) AS rownumb
     ,so.paynights
     ,so.type
     ,so.revolvinggroup
     ,so.addamount
    ...
    FROM
      toospecialoffers so
    WHERE
      (so.itemkey, so.tocode, so.itemtype) = (p_itemkey, p_tocode, p_itemtype)
            ...
            AND so.days =
              Coalesce (
                (
                  ...
                  WHERE
                    (so2.itemkey, so2.tocode, so2.itemtype) = (p_itemkey, p_tocode, p_itemtype)
                    ...
                    AND NOT EXISTS (
                      ...
                      WHERE
                        (so1.itemkey, so1.tocode, so1.itemtype) = (p_itemkey, p_tocode, p_itemtype)
                ...
                (
                  ...
                  WHERE
                    (so3.itemkey, so3.tocode, so3.itemtype) = (p_itemkey, p_tocode, p_itemtype)
      ...
      AND (
       (
         so.ruletype = 'Date' AND p_currentdate BETWEEN so.datebeforedeparturefrom AND so.datebeforedepartureto
       )
       OR (
         so.ruletype = 'Nr'
         AND p_currentdate BETWEEN DATE(p_startdate - so.daysbeforedeparturefrom DAYS) AND DATE(p_startdate - so.daysbeforedepartureto DAYS)
       )
       OR (
         so.ruletype = 'Always'
       )
...
SELECT
  p_startdate + daystart DAYS AS startdate
...
SELECT
  seqdate
  ,ROW_NUMBER() OVER(PARTITION BY childchildnr,daystart ORDER BY seqdate) AS rownum
  ,startdate
  ,enddate
  ,days
  ,daystart
...
SELECT
  ...
  ,ROW_NUMBER()
   OVER (
     PARTITION BY childchildnr, daystart
     ORDER BY price asc, priceall.fromdate desc)  AS priceasc
  ,ROW_NUMBER()
   OVER (
     PARTITION BY childchildnr, daystart
     ORDER BY price desc, priceall.fromdate desc) AS pricedesc
  ,RANK()
   OVER (
     PARTITION BY childchildnr,daystart
...
FROM
  tmprulelist1
  ,TABLE( func_all_tbl( p_tocode, p_itemkey, p_itemtype, p_startdate, p_returndate, p_nradults, p_childbirthdate1, p_childbirthdate2, p_childbirthdate3, p_childbirthdate4 ) ) as priceall
WHERE
