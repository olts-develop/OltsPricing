...
BEGIN
	...
	SELECT p_tocode
		...
		,func_arrpricing(p_tocode, tooarrangement.arrkey, p_startdate, p_enddate, p_currentdate, p_nradults, p_childbirthdate1, p_childbirthdate2, p_childbirthdate3, p_childbirthdate4) AS pricetotal
		,destinationcode
		,arrcode
		,arritemcode
		,func_get_allotment2(p_tocode, tooarrangement.arrkey, 'A', p_startdate, x.ALLOTMENTENDDATE, p_currentdate)
	FROM tooarrangement
		,TABLE (func_arrvalid(p_tocode, tooarrangement.arrkey, p_startdate, p_enddate, p_nradults, p_childbirthdate1, p_childbirthdate2, p_childbirthdate3, p_childbirthdate4)) AS x
...
BEGIN
	...
	SELECT TOCODE
		...
		,TOURBOCODE
		,STATUS
	FROM TABLE (func_arrpricebydest(p_tocode, p_destcode, startdate, enddate, currentdate, p_nradults, childbirthdate1, childbirthdate2, childbirthdate3, childbirthdate4));
END
@
...
BEGIN
	...
	SELECT TOCODE
		...
		,TOURBOCODE
		,STATUS
	FROM TABLE (func_arrpriceav(p_tocode, startdate, enddate, currentdate, p_nradults, childbirthdate1, childbirthdate2, childbirthdate3, childbirthdate4));
END
@
...
BEGIN
	...
	WITH tmptable1(ARRKEY, NRADULTS, CHDDOB1, CHDDOB2, CHDDOB3, CHDDOB4, PRICEENDDATE, ALLOTMENTENDDATE) AS (
			...
			FROM tooarrangement tooarrangement
				,TABLE (func_arrvalid(IN_TOCODE, tooarrangement.ARRKEY, startdate, enddate, IN_NRADULTS, p_childbirthdate1, p_childbirthdate2, p_childbirthdate3, p_childbirthdate4)) AS x
		...
		,tmptable3(ARRKEY, STATUS, TOTAL) AS (
			...
			FROM tmptable2 x
				,TABLE (func_pricing2_tbl(IN_TOCODE, x.ARRKEY, 'A', startdate, x.PRICEENDDATE, currentdate, x.NRADULTS, x.CHDDOB1, x.CHDDOB2, x.CHDDOB3, x.CHDDOB4)) AS pricing
			GROUP BY x.ARRKEY
			)
