CREATE
  ...
  ,IN IN_COUNTRYCODE VARCHAR(2) DEFAULT ''
...
CREATE
	OR REPLACE PROCEDURE SP_PRICING_AV_MISC_STARTDATERELEVANT (
	IN IN_TOCODE VARCHAR(5) DEFAULT ''
	,IN IN_LANGCODE VARCHAR(2) DEFAULT 'DE'
	,IN IN_DESTINATIONCODE VARCHAR(5) DEFAULT ''
	,IN IN_PRICEDATEFROM VARCHAR(10)
...
BEGIN
	DECLARE cursor1 CURSOR WITH RETURN
	FOR
	SELECT TMPDATE AS STARTDATE
		,TOOMISC.TOCODE AS TOCODE
		,TOOMISC.MISCKEY AS MISCKEY
		,TOOMISC.DESTINATIONCODE AS DESTINATIONCODE
		,TOOMISC.MISCCODE AS MISCCODE
		,TOOMISC.MISCITEMCODE AS MISCITEMCODE
		...
		,coalesce(x.STATUS, 'XX') AS STATUS
	FROM TOOMISC
	INNER JOIN TMPDATE on TMPDATE >= IN_PRICEDATEFROM
		and TMPDATE <= IN_PRICEDATETO
	INNER JOIN TABLE (func_miscpriceav2(IN_TOCODE, IN_DESTINATIONCODE, cast(TMPDATE as DATE), cast(TMPDATE + 1 day as DATE), IN_NRADULTS, IN_MISCCODE, IN_MISCITEMCODE, IN_CHDDOB1, IN_CHDDOB2, IN_CHDDOB3, IN_CHDDOB4, IN_IGNORE_XX, IN_IGNORE_RQ, IN_IGNORE_PRICE0, IN_CURRENTDATE, IN_MISCKEY, IN_CURRENCY, IN_EXPORT_ONLY, IN_COUNTRYCODE)) AS x ON x.MISCKEY = TOOMISC.MISCKEY
	LEFT OUTER JOIN TOOMISCTEXT AS INV ON TOOMISC.MISCKEY = INV.MISCKEY
		AND TOOMISC.TOCODE = INV.TOCODE
		AND INV.type = 'INV'
		AND INV.LANG = (
	...
	WHERE x.MISCKEY = TOOMISC.MISCKEY
		and TOOMISC.STARTDATERELEVANT = 1
	ORDER BY TMPDATE
		,TOOMISC.DESTINATIONCODE
		,TOOMISC.MISCCODE
		,TOOMISC.MISCITEMCODE
		,TOOMISC.MISCKEY;
